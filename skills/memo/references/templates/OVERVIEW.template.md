...
| `spec/MEMORY.md` | You need the project memory index. Scan its linked summaries, then read relevant `spec/memory/*.md` entries before relying on them. |
...
2. Scan injected `spec/MEMORY.md`; when a linked summary might affect the task, use Memo memory recall and read the linked entry before deciding.
