...
- Memo 增加按条件加载的知识融合参考：区分记录来源、运行环境、版本演进与真实矛盾，核对两端内容去向及实际读取入口。
- 外部知识改用规范化项目目录名，移除路径哈希身份和新建 project.json；同名 checkout 跨设备共享知识，普通 linked worktree 继续使用主仓库名，无需本机绑定。
- 旧知识副本通过 Memo 合并迁移，Hook 报告待迁移目录并保留原文档；更新存储、迁移、安装说明与跨目录读取回归，CI 增加 macOS 配置。
- Memo 主文明确要求主动将 INDEX 细节拆至 topic/note，并在收尾检查，不等到字节超限。
