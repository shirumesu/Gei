...
### 优化
- 将 GeiSpec 维护调整为两级节奏：日常变更轻量读取注入上下文、记录简洁结果并只更新直接受影响的当前事实；正式版本或 checkpoint 再按版本差异执行完整校准、清理与压缩。
- 为 GeiSpec 增加事件驱动的校准、漂移审计与压缩契约：已验证改动只扫描受影响文档，主动改写或删除过时事实，并补全 Skill 增删改的维护影响矩阵；格式验证器不再把模板 HTML 注释中的示例链接误报为失效链接。
