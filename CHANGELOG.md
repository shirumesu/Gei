...
### Spec 存储与工具
- 修复并发启动时 GitHub 索引刷新持锁超过 400 毫秒导致另一知识 Hook 丢失正文；等待锁上限调整为 4 秒，双宿主知识 Hook 超时同步调整为 10 秒，补充跨进程回归验证。
- 缩短模型可见的版本引用，支持跨进程与重启恢复；新增可选行号读取和基于原始版本的行段替换，大段修改无需重复旧文。匹配失败返回有限上下文和可直接使用的重读参数；修复重叠文本匹配的歧义漏检。补充范围、换行、旧引用兼容和无本地目录的 GitHub 运行验证。
- 默认本地 Markdown，可选连接用户自己的 GitHub 私有仓库；插件捆绑本地 stdio MCP，无需托管服务。支持预览连接/导入、开关、完整缓存刷新和带备份切回本地。
- 新增 read/search/edit/status 工具及等价 CLI。精确文本替换和批量更新校验基础版本，本机多进程协调写入；GitHub 保存直接生成远端提交，无需另行 push。
- Hook 复用存储核心，按需缓存 INDEX 并标记离线状态；停用 Spec 保留数据及其他任务 Skills。更新双宿主打包、安装说明、工具设计依据和一致性测试。
- 修复 Windows Git checkout 的 CRLF/LF 转换导致连接 GitHub 误报冲突；迁移保留已有远端字节，编辑仍要求精确匹配。真实 GitHub 测试扩展至独立存储并发、失败批次、丢失回包、离线和切换行为，并支持验证已安装插件。
