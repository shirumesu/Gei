...
- 优化 Spec 工具与参数说明，明确单文件数组、批量读取与逐文件续读、搜索片段、编辑与复核的联合上限，以及 GC 预览结果到执行参数的对应关系；保持工具形式、参数契约和返回结构不变。
- 参数报错区分缺字段、类型错误和数量／范围错误，为误传 `path` 提供 `paths` 数组示例；补充实际 MCP 发现、错误恢复和批量续读验证。

## v0.11.0.2 - 2026-09-14

- 生命周期 metadata 合回 Markdown 的多行 `gei` frontmatter，由程序通过现有 `reviews` 维护；普通编辑保持已有状态不变，读取、搜索和编辑直接使用完整文件的真实内容与行号。
- 无复核状态的文档保持纯 Markdown，不再仅因缺少 metadata 进入维护候选；明确复核、延期和临时记录销毁继续由统一维护核心处理。
- 独立 metadata 在写入时或通过 `migrate-metadata` 预览后合回文档，保留核验、延期和销毁基准；冲突或损坏记录阻止批量迁移，避免丢失旧状态。
- 新复核状态不再重复保存创建时间和可计算的复核期限；正文及其他 frontmatter 不重新格式化，保留局部 Git diff、换行符与 BOM。
- 补充原始内容分页、实际 Git diff、各旧格式迁移、包内 CLI/MCP、GC 与恢复验证；版本同步和发布说明支持四段开发版本号。

## v0.11.0 - 2026-09-14

- 普通读取与搜索隐藏 Gei 内部维护字段；保留其他 frontmatter，并让读取、搜索、行段编辑、错误定位和 GC 引用共用实际可见内容的行号，兼容没有 metadata 的旧文档。
- 普通创建和修改自动建立未复核状态，无需附加 review，也不会续期整篇知识。完整复核统一使用可选的 `reviews: [{path, outcome: "verify", basis}]`；`basis` 合并原有 reason/evidence，支持引用正文中的证据。原 keep/update 复核参数由 verify 替代。
- MCP 维护检查与 GC 返回可读条目；CLI check/gc 默认使用相同格式，脚本可加 `--json` 获取结构化结果。维护清单不输出内部内容指纹。
- 新复核基线覆盖正文及有意义的 frontmatter；旧生命周期状态保持可读，历史删除声明不自动续期。本地删除恢复原样还原正文及维护状态。
- 精简 Memo 维护指引，区分局部修正、完整复核和证据不足时延期；补充混合文件格式、分页、恢复及 CLI/MCP 回归验证。

## v0.10.4 - 2026-09-14

- 修复定时清理模板在 job env 中使用不受支持的 runner 上下文导致 GitHub 拒绝启动的问题，改为在运行步骤中配置临时状态目录。
- 修复连接 GitHub 时将工作流令牌的权限信息误判为不可写的问题；连接及后续操作的权限由实际 GitHub API 响应确认。
- 定时清理模板固定使用 v0.10.4，包含工作流运行和令牌兼容修复。

## v0.10.3 - 2026-09-14

- 修复维护检查将不同项目的同名 README/INDEX 误判为入站依赖的问题；文本引用按文档相对路径或知识根路径匹配，保留真实跨项目引用的删除保护。
- 更新安装验证中的 MCP 工具清单，并补充官方 SDK 对结构化维护、GC 预览和计划校验的互通检查。
- 随包提供的定时清理模板固定使用 v0.10.3，包含本次引用检查修复。

## v0.10.2 - 2026-09-14

### 知识维护
- 新增统一 CLI/MCP 的 `spec_check` 和 `spec_gc`：维护清单包含原因、证据入口和引用，复核日期只触发检查；低频项目、普通知识和交接不会按年龄自动删除。
- `spec_edit` 支持保留、更新、删除和延期复核，程序维护验证时间、内容版本、条件证据与环境范围；旧 Markdown 保持可读，缺少元数据不会被当作过期。
- 临时记录只有明确声明销毁日期、内容未变化且没有实质依赖才可自动清理。执行校验预览版本和计划，批量修复导航；本地删除提供有限期恢复，GitHub 保存仍是原子提交。
- Memo 增加按条件加载的维护指引，覆盖到期后的实际处理、交接知识提取、证据不足时延期及只预览边界；启动只给有节制的范围提示，不增加归档层或强制 Stop 审核。
- 分发包包含默认预览的定时 GC runner 和可选 GitHub Actions 模板，不自动安装或启用用户知识库的清理任务。

### 验证
- 增加维护、引用依赖、恢复、并发、离线和实际 CLI/MCP 回归；两个解压后的发行包均检查维护入口。
- 验证范围和真实知识检查边界见 `docs/verification.md`；运行测试不代表 AI 能自动证明所有知识的语义有效性。

## v0.10.1

- 简化已注入 INDEX 的复用指引，普通读取直接跟随相关路由，避免为获取相同背景重复读取 INDEX。

## v0.10.0 - 2026-09-12

### Spec 存储与工具
- 默认使用本地 Markdown，可选连接用户自己的 GitHub 私有知识仓库。Codex 与 Claude Code 插件捆绑本地 stdio MCP，无需部署服务；Skills 压缩包也包含独立 Spec CLI。
- 新增 `spec_read`、`spec_search`、`spec_edit`、`spec_status` 及等价 CLI，支持固定版本读取、精确文本替换和多文件批量更新。写入校验基础版本并协调本机多进程；GitHub 保存直接生成远端提交，无需另行 push。
- 支持连接/导入预览、Spec 开关、完整缓存刷新和带备份切回本地。Hook 复用存储核心，按需缓存 INDEX 并标记离线状态；停用 Spec 保留数据及其他任务 Skills。

### 编辑体验
- 将版本引用缩短为可跨进程和重启复用的短引用，兼容已有完整引用。
- 新增可选行号读取和基于原始版本的行段替换，大段修改无需重复旧文；匹配失败返回有限上下文和可直接使用的重读参数。

### 修复与验证
- 修复 Windows Git checkout 的 CRLF/LF 转换导致连接 GitHub 误报冲突；迁移保留已有远端字节，日常编辑仍要求精确匹配。
- 修复并发启动时 GitHub 索引刷新持锁超过 400 毫秒导致另一知识 Hook 丢失正文；等待锁上限调整为 4 秒，双宿主知识 Hook 超时同步调整为 10 秒。
- 修复重叠文本匹配的歧义漏检，补充行段范围、换行保留、旧引用兼容、跨进程并发和无本地知识目录的 GitHub 运行回归。
- 发布检查覆盖两个实际压缩包的解压后 CLI 与插件 Hook；扩展存储、MCP 协议及可选真实 GitHub 验证，涵盖失败批次、丢失回包、离线读取和后端切换。

### 升级说明
- 运行 Hook、MCP 或 CLI 需要 Node.js 22 或更新版本；无需 npm install。Markdown 目录结构及项目命名保持不变，安装不会自动连接或上传到 GitHub。
- 日常知识维护改用 Spec 工具；GitHub 模式不再另行执行本地 Git 同步，离线时只读已缓存内容。连接、认证和切回本地的操作见 [Spec 使用说明](https://github.com/shirumesu/Gei/blob/v0.10.0/docs/spec.md)。

## v0.9.1 - 2026-09-11

### 跨设备知识
- 外部知识改用规范化项目目录名，移除路径哈希身份和新建 `project.json`；同名 checkout 跨设备共享知识，普通 linked worktree 继续使用主仓库名，无需本机绑定。
- 更新存储、迁移及中英文安装说明。知识库可独立同步；Hook 负责读取，不代替用户执行 Git 同步。

### Memo 维护
- 明确要求主动将 INDEX 中的主题细节拆至 topic/note，保留简短检索入口，并在收尾检查，不等到字节超限。
- 增加按条件加载的知识融合参考：区分记录来源、运行环境、版本演进与真实矛盾，保留适用范围，核对两端内容去向及实际读取入口。

### 修复与验证
- 发布包纳入 v0.9.0 发布后的 Windows 短路径身份修复，以及避免分支与标签重复触发 Check 的调整。
- 补充同名 checkout、复制知识库、跨系统旧清单等 Hook 回归，CI 验证范围扩展至 Windows、Linux 和 macOS。

### 升级说明
- 从旧路径哈希目录升级时，先通过 Memo 合并旧知识到 `projects/<project-name>`；Hook 会报告待迁移目录并保留原文档，不自动选择或覆盖冲突内容。
- 两端项目目录名需一致；规范化后同名的项目会共享知识。代码与私人知识仓库分别 push/pull，目标项目无需提交 `.agents` 或 Spec。
