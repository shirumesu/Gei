...
### Skill 优化
- Code Review 优先检查功能、交互、呈现、性能与产品/代码一致性；区分缺陷和有依据的改善建议，收紧安全参考的触发范围并精简内容。
- Consider 主动提出可执行的竞争方案，用反例挑战自身推荐并修订设计；按决定之间的依赖追问，避免将设计工作转交给用户或重复审批。
- Work 聚焦入口到实际消费者的完整路径、因果排查和能区分不完整修复的验证；删除通用执行清单，保留按需测试与发布参考。
- 同步任务路由和中英文能力列表，合并 Consider 的重复上下文参考。

### 重大调整
- 将 GeiSpec 重设计为项目外部的精简 INDEX、按领域展开的 Topics、带适用条件的决策/踩坑 Notes 与必要交接；取消固定五件套、Group 注册和默认内部 Changelog。
- 将七条 SessionStart Hook 收缩为独立的任务路由与知识入口；启动只读，限制 UTF-8 输出并保留完整检索行，详细领域资料按需获取。
- Agent 在背景明确、取舍被接受、可复用坑被验证、资料失效或需要交接时自主维护，无需单独询问；Consider 区分用户决定与推断，并重新核对历史取舍的适用条件。
- Git 子目录和 linked worktree 共享知识身份，保留当前 checkout 用于核对分支证据；旧资料提供按需迁移入口，保持原始内容直至完成核对。

### 验证
- 添加 Hook 路径、只读启动、旧资料保留、索引预算和宿主配置回归；CI 覆盖 Windows/Linux 及全部 Skill 格式。
- 保留格式验证器对模板注释示例的过滤，避免将示例链接误报为失效链接。
