...
`spec_edit` also accepts structured `reviews` alongside edits (or without text changes). Ordinary edits initialize state without renewing verification; optional `verify` reviews explicitly confirm the whole resulting document. Read/search/edit coordinates address content with the reserved lifecycle field removed; semantic deletion and link repairs remain one atomic operation. See [knowledge maintenance](maintenance.md) for outcome schemas, legacy initialization, check/GC CLI flags, deletion recovery, and optional schedules.
