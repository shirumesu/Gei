...
`spec_edit` also accepts structured `reviews` alongside edits (or without text changes). Ordinary edits initialize state without renewing verification; optional `verify` reviews explicitly confirm the whole resulting document. Read/search/edit coordinates address the stored Markdown verbatim; lifecycle state is maintained under the Spec root’s `metadata/` directory. Semantic deletion and link repairs remain one atomic operation. See [knowledge maintenance](maintenance.md) for outcome schemas, legacy initialization, check/GC CLI flags, deletion recovery, and optional schedules.
