...
MCP checks also verify complete tool definitions survive discovery, parameter errors identify corrective input shapes and bounds, and batch reads can resume each file on the same snapshot. These deterministic checks establish the protocol behavior; first-call accuracy and fewer model recovery calls require separate agent evaluation.
...
Maintenance tests cover physical Markdown coordinates before and after migration for legacy, mixed and malformed frontmatter; CRLF/BOM preservation; raw long metadata pagination; exact/range recovery context; explicit whole-document verification; formatted CLI/MCP output; legacy metadata, non-destructive review dates, content changes without automatic renewal, deferred verification, source and environment scope, fixed-revision pagination, CLI/MCP check and GC, dependency-aware deletion, transient retention invalidated by body changes, stale plans, local backup/restore, and remote atomic cleanup with offline and lost-response behavior. Migration checks compare pre/post verification, deferral and GC eligibility across the three deployed hash formats; exercise explicit CLI migration, whole-file replacements, metadata-only concurrency, single-file rename/delete recovery, and lost remote responses; and verify ordinary edits leave the managed block unchanged while reviews update it in the same Markdown blob. Actual Git diff checks distinguish tail-only edits from combined header/tail updates. No-metadata documents remain plain and do not become candidates solely for missing state. Package checks run migration, edits, maintenance and GC through both extracted entrypoints. Tests use isolated stores; an actual user's knowledge must be checked read-only unless cleanup is separately authorized.

Skill behavior checks should use a fresh agent with Memo and its maintenance reference: preserve unchanged low-frequency knowledge; defer unavailable environment or completion evidence; retain unique requirements when merging; repair substantive dependencies before deletion; honor preview-only requests; and avoid archive searches in ordinary work. An isolated CLI exercise can verify actual verify/defer/GC behavior. These examples establish behavior in those cases, not general semantic accuracy or a guarantee against every informal dependency.
