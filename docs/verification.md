...
Maintenance tests cover shared visible coordinates for legacy, managed, mixed and malformed frontmatter; CRLF/BOM preservation; hidden long metadata; exact/range recovery context; explicit whole-document verification; formatted CLI/MCP output; legacy metadata, non-destructive review dates, content changes without automatic renewal, deferred verification, source and environment scope, fixed-revision pagination, CLI/MCP check and GC, dependency-aware deletion, transient retention invalidated by body changes, stale plans, local backup/restore, and remote atomic cleanup with offline and lost-response behavior. Package checks run check/GC previews through both extracted entrypoints. Tests use isolated stores; an actual user's knowledge must be checked read-only unless cleanup is separately authorized.
