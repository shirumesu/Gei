...
- `keep` verifies existing content and requires an evidence list.
- `update` accompanies a creation, correction, or merge and requires evidence.
- `delete` deletes that record; repair incoming references with edits in the same batch.
- `defer` records missing evidence without renewing verification. The same content/evidence condition cools for 30 days; new body or source observations can reopen it.

Timestamps and content hashes are generated by the runtime. Example CLI input (use an actual read/check revision):
...
```json
{
  ...
  "reviews": [{
    ...
    "outcome": "keep",
    "reason": "The current compatibility contract still requires this behavior",
    "evidence": ["docs/compatibility.md and the current reader implementation"],
    "review_days": 365
  }]
}
```
...
The runtime reserves one frontmatter field, `gei: {"version":1,...}`, using JSON as a YAML value. Other frontmatter and body text are preserved. Create new earned records with an `update` review to initialize lifecycle information. Existing raw create/edit calls keep their byte-preserving contract and do not fabricate verification; legacy files remain readable and become initialization candidates.

Kinds are `knowledge` (default), `handoff` (default under tasks/), and `transient`. Default review intervals are 90, 14, and 30 days respectively, configurable from 1 to 365. Environment observations normally use knowledge with a 30-day review interval. Stable accepted requirements can use 365 days. These are review intervals, not deletion deadlines. Read/search/import, ordinary edits, and opening a project never renew verification. The read response reports lifecycle information separately from transport/cache `stale`.
